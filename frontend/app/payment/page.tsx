...
function PaymentContent() {
  ...
  // Кнопка "Я уже оплатил" — быстрая проверка, если не подтверждено → всё равно показываем успех
  // Пользователь видел "Успешно!" в Robokassa — доверяем, webhook дойдёт позже
  const handleManualCheck = useCallback(async () => {
    if (!currentInvId) return;
    setCheckingStatus(true);
    // Одна быстрая проверка — вдруг webhook уже дошёл
    ...
    if (paid) {
      ...
      // Доверяем пользователю — показываем успех
      // Webhook от Robokassa дойдёт до backend в фоне
      setPaymentStatus('success');
    }
